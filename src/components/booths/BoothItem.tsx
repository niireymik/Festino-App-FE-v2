    ...
    <div className="w-full pb-20 dynamic-padding">
      {currentBoothList.length === 0 ? (
        ...
        currentBoothList.map((booth, index) => {
          ...
          const boothImageProps = getBoothImageProps(booth.boothImage);

          return (
            <div
              key={index}
              className="pb-2 cursor-pointer"
              onClick={() => handleClickBoothIntroduction(booth.adminCategory, booth.boothId)}
            >
              <div className="w-full h-[160px] bg-white shadow-4xl flex flex-row justify-between items-center rounded-2.5xl border border-primary-900-light-16 px-4 py-3">
                <div className="w-[222px] h-full flex flex-col justify-between pr-1">
                  <div>
                    ...
                    <div>
                      <div className="py-1.5 text-base font-semibold">{booth.boothName}</div>
